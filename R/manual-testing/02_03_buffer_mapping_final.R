...
# testing for joining networks =================================================

small_chanel <- sf::st_crop(
  utm_geo_data,
  ymin = 5812000, ymax = 5830000, xmin = 535000, xmax = 555000
)

ggplot() + 
  geom_sf(data = small_chanel, color = 'black', fill = "grey90") +
  coord_sf(datum = "+proj=utm +zone=9 +datum=NAD83 +unit=m")+
  theme_base()


grid_sample <- sf::st_sample(
  sf::st_as_sfc(sf::st_bbox(small_chanel)),
  # the size is really large to make a fine grid
...
large_network <- as_sfnetwork(grid_cropped, directed = FALSE) %>% 
  activate("edges") %>% 
  mutate(weight = edge_length())

# challenging area
small_chanel2 <- sf::st_crop(
  utm_geo_data,
  ymin = 5819000, ymax = 5821000, xmin = 547300, xmax = 547800
)
...
pts = sf::st_sample( 
  ...
  sf::st_as_sf() %>%
  nngeo::st_connect(.,.,k = 9) 
pts_cropped <- pts[sf::st_contains(small_chanel, pts, sparse = F)]

check_crop <- st_union(x = grid_cropped, y = pts_cropped)

ggplot() +
  geom_sf(data = small_chanel) + 
  geom_sf(data = check_crop) +
  #geom_sf(data = check_net %>% activate("edges") %>% st_as_sf()) +
 # geom_sf(data = check_net %>% activate("nodes") %>% st_as_sf()) +
  coord_sf(ylim = c(5819000,5821000), xlim = c(546000, 549000),
    datum = "+proj=utm +zone=9 +datum=NAD83 +unit=m")
...
network <- sfnetworks::st_network_join(large_network %>% mutate(net = "large"),
                                       small_network %>% mutate(net = "small"), 
                                       blend_nodes = TRUE)

ggplot() + 
  geom_sf(data = small_chanel, color = 'black', fill = "grey90") +
  geom_sf(data = large_network %>% activate("edges") %>% st_as_sf()) +
  geom_sf(data = large_network %>% activate("nodes") %>% st_as_sf()) +
  theme_base() + 
  coord_sf(xlim=c(540000, 550000), ylim=c(5815000, 5825000),
           datum = "+proj=utm +zone=9 +datum=NAD83 +unit=m")
...
ggplot() + 
  geom_sf(data = small_chanel, color = 'black', fill = "grey90") +
  geom_sf(data = large_network %>%
            activate("nodes") %>%
            slice(nodes_to_keep_jackson) %>% 
            st_as_sf(), fill = "lightpink", colour = "lightpink") +
  geom_sf(data = large_network %>%
            activate("edges") %>%
            slice(keep_edges_jackson) %>% 
            st_as_sf(), fill = "lightpink", colour = "lightpink") +
  geom_sf(data = jackson,
          shape = 21, fill = "purple1", colour = "black", size = 2.5) +
  theme_base() + 

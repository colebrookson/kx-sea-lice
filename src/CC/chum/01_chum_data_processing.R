...
files <- list.files(here("./outputs/power-analysis/saved-runs/chum/"))
...
for(f in files) {
  ...
  df_temp <- readr::read_csv(paste0(
    here("./outputs/power-analysis/saved-runs/chum/"), 
    f),
...
readr::write_csv(
  df, 
  here("./outputs/power-analysis/chum-all-power-analysis-runs.csv")
)
